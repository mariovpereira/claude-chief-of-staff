...
mkdir -p "$CLAUDE_DIR/contacts/investors"
mkdir -p "$CLAUDE_DIR/commands"
mkdir -p "$CLAUDE_DIR/objectives"
mkdir -p "$CLAUDE_DIR/task-outputs"
mkdir -p "$CLAUDE_DIR/investors"
mkdir -p "$CLAUDE_DIR/fundraising"
mkdir -p "$CLAUDE_DIR/team"
mkdir -p "$CLAUDE_DIR/product"
mkdir -p "$CLAUDE_DIR/accountability"
...
copy_if_missing "$SCRIPT_DIR/contacts/investors/_template.md" "$CLAUDE_DIR/contacts/investors/_template.md"

# Copy founder workspace files
copy_if_missing "$SCRIPT_DIR/judgment.yaml" "$CLAUDE_DIR/judgment.yaml"
copy_if_missing "$SCRIPT_DIR/investors/_template.md" "$CLAUDE_DIR/investors/_template.md"
copy_if_missing "$SCRIPT_DIR/fundraising/overview.md" "$CLAUDE_DIR/fundraising/overview.md"
copy_if_missing "$SCRIPT_DIR/fundraising/pipeline.md" "$CLAUDE_DIR/fundraising/pipeline.md"
copy_if_missing "$SCRIPT_DIR/fundraising/materials.md" "$CLAUDE_DIR/fundraising/materials.md"
copy_if_missing "$SCRIPT_DIR/team/alan.md" "$CLAUDE_DIR/team/alan.md"
copy_if_missing "$SCRIPT_DIR/product/roadmap.md" "$CLAUDE_DIR/product/roadmap.md"
copy_if_missing "$SCRIPT_DIR/product/decision-log.md" "$CLAUDE_DIR/product/decision-log.md"
copy_if_missing "$SCRIPT_DIR/accountability/commitments.md" "$CLAUDE_DIR/accountability/commitments.md"
...
echo "  commands/          — Skill definitions (11 commands including Claudia-inspired additions)"
echo "  judgment.yaml      — Learned decision rules (grows over time)"
echo "  investors/         — Investor profile templates"
echo "  fundraising/       — Round overview, pipeline, materials checklist"
echo "  team/              — Team profiles"
echo "  product/           — Roadmap and decision log"
echo "  accountability/    — Commitment tracking"
echo ""
